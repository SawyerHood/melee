...
static inline void lbAudioAx_BusyWait(void)
{
    s32 n;
    s32 accum;
    s32 j;
    ...
    if (gobj != NULL) {
        ...
        if (ud != NULL) {
            if (1.0f == ud->x3C) {
                ...
                ud->x2C.pan =
                    0x7F - fn_800250A0(ud->current_frame, ud->end_frame,
                                       ud->pan_left, ud->pan_right);
            }
        }
    }
        ...
        if (ud != NULL) {
            if (ud->current_frame <= ud->end_frame) {
                if (ud->start_val < ud->end_val) {
                    if ((diff = (f32) ud->end_val - (f32) ud->start_val) <
                        0.0f)
                    {
                        diff = -diff;
                    }
...
found:
    if (slot != -1) {
        s32 idx = slot * 4;
        char* q = lbl_803BB300 + idx;
        strcpy(&lbl_803BB340[lbl_804D38D0], *(char**) (q + 0x9FC));
...
void lbAudioAx_80027DF8(void)
{
    ...
    if (lbl_804D6420 != 0) {
        if (lbl_804D38F0 != -1) {
            if (AXDriver_8038D9D8(lbl_804D38F0) == 0) {
                ...
                AXDriver_8038D3B8(id, (vol * 2) & 0xFE);
            }
        } else {
            lbl_804D6428 = 0x7F;
        ...
        lbl_804D6428 = 0;
        lbl_804D38F0 = -1;
        lbl_804D642C = (lbl_804D38F4 == -1) ? 0 : 0x7F;
    }
    ...
    if (lbl_804D6424 != 0) {
        ...
        lbl_804D642C = 0;
        lbl_804D38F4 = -1;
        lbl_804D6428 = (lbl_804D38F0 == -1) ? 0 : 0x7F;
    }
...
s32 lbAudioAx_80028690(void)
{
    ...
    if (lbl_804D3878 != var_r29) {
        ...
        {
            int* p = &lbl_80433710.x274[2];
            if (lbl_80433710.x274[2] < 1) {
                strcpy(&lbl_803BB340[lbl_804D38D0],
