...
static inline bool ftCo_800A2718_onett(enum InternalStageId* stage, float f)
{
    if (*stage != ONETT) {
        return false;
    }
    if (f <= 5.0 && Ground_801C5794() != 0) {
        return true;
    }
    return false;
}
...
{
    Item_GObj* cur;
    mp_UnkStruct0* island = (mp_UnkStruct0*) ((u8*) arg0 + 0);
    Item* cur_ip;
    enum InternalStageId* stage;
    f32 y;
    UNUSED u8 pad[0x18];
    ...
    while (cur != NULL) {
        ...
        if (it_8026C1B4(cur) != 0 && ftCo_800A5944(cur_ip) &&
            island == mpIsland_8005AB54(cur_ip->x378_itemColl.floor.index))
        {
            return true;
        }
        cur = cur->next;
    ...
    case ZEBES:
        ...
        return false;
    case ONETT:
        if (ftCo_800A2718_onett(stage, island->x14.y) ||
            ftCo_800A2718_onett(stage, island->x8.y))
        {
            return true;
        }
        return false;
...
Fighter* ftCo_800A4BEC(Fighter* fp)
{
    ...
    if (closest == NULL) {
        ...
        data->xF9_b0 = true;
        dist = HSD_Randf();
        data->x30 = dist * 0.5f * 0.5f * 120.0f;
        data->x48 = closest;
    }
    return closest;
}
...
static inline void ftCo_800AF290_tail(Fighter* fp, Fighter** pp)
{
    Fighter* other_fp = *pp;
    if (other_fp != NULL && fp->ground_or_air != GA_Air) {
        Vec3 vec;
        ...
        if (!(dist > 50.0) &&
            ftCo_800A6700(fp, &other_fp->cur_pos, &vec) != 0)
        {
            ftCo_800A1F3C(fp, vec.x, vec.y, 5.0f);
        }
    }
    ...
    if (inlineI1_alt(fp)) {
        if (temp_r30->x4C != NULL) {
            ...
            if (temp_r4_2 != NULL && fp->ground_or_air != GA_Air) {
                ...
                if (!(ftCo_800AF78C_sqrtf(temp_f2_2 * temp_f2_2 +
                                          temp_f3 * temp_f3) > 50.0) &&
