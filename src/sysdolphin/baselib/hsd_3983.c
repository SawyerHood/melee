 ...
 * library that had been merged into particle.c: the particle system
 * proper (the DOL's own assert strings "particle.c" at .data 0x8040BFB0
 * date this TU). TU evidence: own .sdata2 literal pool at 0x804DE948
 * (dup @391/@535/@541/@908 copies), .bss base hsd_804D08E8 (one of the
 * two bases demanding section offset 0), extab 0x80005520, extabindex
 * 0x800056C0. Derivation: campaign/scratch/tu-split-particle/.
 */
 ...
 * form tables, D0D58 "psCmdListArray" the per-bank counts, D0E5C
 * "ptclref_804D0E5C" the per-bank HSD_PSCmdList* tables; the spawn
 * remap table lives at +0x60 off hsd_804D08E8 (stored from `ref`). */
/* 4D0C54 */ static int* psNumCmdList_804D0C54[65];
/* 4D0D58 */ static int psCmdListArray[65];
/* 4D0E5C */ static HSD_PSCmdList** ptclref_804D0E5C[65];
/* 4D0F60 */ struct hsd_804D0F60_t hsd_804D0F60;

extern int psNumTexGroup[65];

/* .sbss (declaration order fixes the section layout) */
        ...
        if (opcode < 0x80) {
            ...
            case 0xF1:
                /* Spawn child particle from ptclref remap */
                {
                    ...
                    if (((u32**) ((s32*) hsd_804D08E8 + bank))[0x60 / 4] != NULL) {
                        idx = ((u32**) ((s32*) hsd_804D08E8 +
                                        bank))[0x60 / 4][idx];
                    }
            ...
            case 0xF0:
                /* Spawn generator with kind flags + ptclref remap */
                {
                    ...
                    if (((u32**) ((s32*) hsd_804D08E8 + pp->bank))[0x60 / 4] != NULL) {
                        idx = ((u32**) ((s32*) hsd_804D08E8 +
                                        pp->bank))[0x60 / 4][idx];
                    }
